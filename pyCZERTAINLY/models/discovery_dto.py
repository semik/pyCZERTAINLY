...
class DiscoveryDto(BaseModel):
    ...
    DiscoveryDto
    """ # noqa: E501
    name: StrictStr = Field(description="Discovery name")
    attributes: List[RequestAttributeDto] = Field(description="List of Attributes for Discovery")
    ...
    connector_uuid: StrictStr = Field(description="Discovery Provider UUID", alias="connectorUuid")
    kind: StrictStr = Field(description="Discovery Kind")
    triggers: Optional[List[StrictStr]] = Field(default=None, description="List of triggers to be triggered after the discovery is finished, triggers will be evaluated in given order")
    __properties: ClassVar[List[str]] = ["name", "attributes", "customAttributes", "connectorUuid", "kind", "triggers"]
