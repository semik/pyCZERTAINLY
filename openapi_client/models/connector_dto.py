...
class ConnectorDto(BaseModel):
    ...
    function_groups: List[FunctionGroupDto] = Field(description="List of Function Groups implemented by the Connector", alias="functionGroups")
    url: StrictStr = Field(description="URL of the Connector")
    auth_type: AuthType = Field(alias="authType")
    auth_attributes: Optional[List[ResponseAttributeDto]] = Field(default=None, description="List of Attributes for the authentication type", alias="authAttributes")
    ...
    __properties: ClassVar[List[str]] = ["uuid", "name", "functionGroups", "url", "authType", "authAttributes", "status", "customAttributes"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in function_groups (list)
        _items = []
        if self.function_groups:
            for _item_function_groups in self.function_groups:
                if _item_function_groups:
                    _items.append(_item_function_groups.to_dict())
            _dict['functionGroups'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "uuid": obj.get("uuid"),
            "name": obj.get("name"),
            "functionGroups": [FunctionGroupDto.from_dict(_item) for _item in obj["functionGroups"]] if obj.get("functionGroups") is not None else None,
            "url": obj.get("url"),
            "authType": obj.get("authType"),
            "authAttributes": [ResponseAttributeDto.from_dict(_item) for _item in obj["authAttributes"]] if obj.get("authAttributes") is not None else None,
